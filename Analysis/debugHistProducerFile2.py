...
#yumeng
def createCentralQuantities(df_central, central_col_types, central_columns):
    #print("Creating MapCreator with types:", central_col_types)
    try:
        map_creator = ROOT.analysis.MapCreator(*central_col_types)()
        print("MapCreator success.")
    except Exception as e:
        print("MapCreator fail:", e)
        raise

    #print("Processing DataFrame with columns:", central_columns)
    try:
        df_central = map_creator.processCentral(ROOT.RDF.AsRNode(df_central), Utilities.ListToVector(central_columns), 1)
        print("processCentral success.")
        cols = list(df_central.GetColumnNames())
        #print("Columns in processed DataFrame:", cols)
        if "map_placeholder" in cols:
            print("map_placeholder column exists.")
        else:
            print("no map_placeholder")
    except Exception as e:
        print("processCentral fail:", e)
        raise
...
if __name__ == "__main__":
    ...
    print("debug0 Parsed args:", vars(args))
    ...
    print("debug1 includes and interpreter")
    ...
    print("debug2 Loaded Config YAMLs")
    ...
    key_not_exist = False
    print("debug3 Opening input file:", args.inFile)
    df_empty = False
    ...
    if not key_not_exist and ROOT.RDataFrame('Events',args.inFile).Count().GetValue() == 0:
        df_empty = True
    print(f"debug4 key_not_exist = {key_not_exist}, df_empty = {df_empty}")
    ...
    if not create_new_hist:
        ...
        print(f"debug5 before dfWrapped_central")
        ...
        # dfyumeng = ROOT.RDataFrame("Events", args.inFile)
        # print("Number of entries in Events:", dfyumeng.Count().GetValue())
        # print("Defined columns:", dfWrapped_central.colNames)
        # print("Column types   :", dfWrapped_central.colTypes)
        # print("Number of entries:", dfWrapped_central.df.Count().GetValue())
        # dfWrapped_central.df.Display("tau1_pt").Print()
        print(f"debug6 after dfWrapped_central")

        key_central = (args.dataset, "Central", "Central")
        key_filter_dict = analysis.createKeyFilterDict(global_cfg_dict, args.period)

        print("debug7 Created key_filter_dict")

        outfile  = ROOT.TFile(args.outFileName,'RECREATE')

        print("debug8 open output file")
        ...
        print("debug10 PrepareDfForHistograms")
        ...
        print("debug11 GetHistogramDictFromDataframes")
        ...
        if compute_variations:
            ...
            print("debug11.5")
            # try:
            #     print("Type of map_placeholder:", all_dataframes[key_central][0].GetTypeName("map_placeholder"))
            # except Exception as e:
            #     print("error: Exception caught in GetTypeName:", e)
            
            # if all_dataframes[key_central][0].Count().GetValue() <= 0:
            #     raise RuntimeError("no events passed map placeholder")
            
            # try:
            #     df_test_np = all_dataframes[key_central][0].AsNumpy(["map_placeholder"])
            #     print("First 5 values of map_placeholder:", df_test_np["map_placeholder"][:5])
            # except Exception as e:
            #     print("error: AsNumpy failed:", e)

            cols = list(all_dataframes[key_central][0].GetColumnNames())
            print("map_placeholder" in cols)

            # try:
            #     all_dataframes[key_central][0].Display("map_placeholder").Print()
            # except Exception as e:
            #     print("error: Display failed:", e)

            # for col in cols:
            #     try:
            #         t = all_dataframes[key_central][0].GetTypeName(col)
            #         print(f"{col}: {t}")
            #     except Exception:
            #         print(f"{col}: unknown or unsafe type")

            print("debug11.7 nruns",all_dataframes[key_central][0].GetNRuns())
            ...
            print("debug12 createCentralQuantities")
        ...
        print("debug13 Central histograms saved")
