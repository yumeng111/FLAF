...
class PlotTask(Task, HTCondorWorkflow, law.LocalWorkflow):
    ...
    def run(self):
        ...
        if plot_unc:
            #If downstream task requires your task, Law automatically plug your remote_target(...) into their self.input().
        ...
        #Downloads remote files to local temp file. Ensures code works on local and Condor (mainly for Condor, will likely fail because they can’t access remote paths). Cleans up temp file automatically after use
        with remote_in.localize("r") as local_input:
            ...
            with self.output().localize("w") as local_flag_path:
